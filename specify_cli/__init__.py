...
def download_template_from_github(ai_assistant: str, download_dir: Path, *, script_type: str = "sh", verbose: bool = True, show_progress: bool = True, client: httpx.Client = None, debug: bool = False, github_token: str = None) -> Tuple[Path, dict]:
    # Use configured repository (supports forks and custom repos)
...
def initialize_context_file(project_path: Path, tracker: StepTracker | None = None) -> None:
    ...
    try:
        ...
        if tracker:
            tracker.complete("context-init", "workflow state tracking ready")
    except Exception as e:
        if tracker:
            tracker.error("context-init", str(e))
        else:
            console.print(f"[yellow]Warning: Could not create context file: {e}[/yellow]")


def report_next_steps(project_path: Path, ai_assistant: str) -> None:
    """Display next steps to the user after initialization."""
    workflow_dir = PLATFORM_WORKFLOW_DIRS.get(ai_assistant, None)
    
    next_steps = Panel(
        f"""[bold cyan]Project initialized successfully![/bold cyan]

[white]Next steps:[/white]

1. [cyan]Navigate to your project:[/cyan]
   cd {project_path.name if project_path.name != '.' else 'your-project'}

2. [cyan]Start with the /specify workflow:[/cyan]
   {"Use: /specify in " + ai_assistant if workflow_dir else "Open .specify/templates/commands/specify.md"}
   
3. [cyan]Review the constitution:[/cyan]
   Check .specify/memory/constitution.md for project principles

4. [cyan]Explore available workflows:[/cyan]
...
[dim]Tip: All workflows are in {workflow_dir if workflow_dir else '.specify/templates/commands/'}[/dim]
""",
        title="🎉 Ready to Go",
        border_style="green",
        padding=(1, 2),
...
def init(
    ...
    with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
        ...
        try:
            ...
            # Initialize context.json for workflow state tracking
    ...
    # Display enhanced next steps with workflow information
    report_next_steps(project_path, selected_ai)
